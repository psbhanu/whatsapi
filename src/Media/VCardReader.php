<?php
namespace psbhanu\Whatsapi\Media;
...
/**
 * VCardReaderHelper class for parsing a vCard and/or creating one
 *
 ...
 * @version 0.4.8
*/
class VCardReader implements Countable, Iterator
{
	...
	public function __construct($Path = false, $RawData = false, array $Options = null)
	{
		...
		{
			$this->Mode = VCardReaderHelper::MODE_ERROR;
			throw new Exception('vCard: invalid vCard');
		}

		$this->Mode = $vCardBeginCount == 1 ? VCardReaderHelper::MODE_SINGLE : VCardReaderHelper::MODE_MULTIPLE;
	...
	public function __call($Key, $Arguments)
	{
		...
		if (count($Arguments) > 1)
		{
			...
			if (isset(self::$Spec_StructuredElements[$Key]) && in_array($Arguments[1], self::$Spec_StructuredElements[$Key]))
			{
				$LastElementIndex = 0;
	...
	private static function ParseParameters($Key, array $RawParams = null)
	{
		...
		foreach ($Parameters as $Index => $Parameter)
		{
			...
			if (count($Parameter) == 1)
			{
				...
				if ((isset(self::$Spec_ElementTypes[$Key]) && in_array($Parameter[0], self::$Spec_ElementTypes[$Key])) || ($Key == 'email' && is_scalar($Parameter[0])))
				{
					$Type[] = $Parameter[0];
				}
			}
	...
	public function parse(VCardReaderHelper $vCard)
	{
		$card = new stdClass();

		$card->fn = $vCard->FN[0];

		foreach ($vCard as $index => $vcard)
		{
			$index = strtolower(preg_replace('/\W+/i', '', $index));

			if(is_scalar($vcard))
			{
				$card->$index = $vcard;
			}
			elseif(is_array($vcard))
			{
				$tmp = array();

				if(count($vcard) > 1)
				{
					foreach ($vcard as $subindex => $value)
					{
						if(is_scalar($value))
						{
							$tmp = $value;
				...
				$card->$index = is_array($tmp) ? (object) $tmp : $tmp;
			}
		}
		...
		if ($vCard->PHOTO)
		{
			foreach ($vCard->PHOTO as $photo)
			{
				if ($photo['Encoding'] == 'b')
				{
					$card->photo[] = '<img src="data:image/'.$photo['Type'][0].';base64,'.$photo['Value'].'">';
				}
				else
				{
					$card->photo[] = '<img src="'.$photo['Value'].'">';
				}
			}
		}
		...
		{
			$card->name[] = $name['FirstName'].' '.$name['LastName'].'</h3>';
		}

		foreach ($vCard->ORG as $organization)
		{
			$card->organization[] = $organization['Name']. ($organization['Unit1'] || $organization['Unit2'] ? ' ('.implode(', ', array($organization['Unit1'], $organization['Unit2'])).')' : '');
		}
		...
		if ($vCard->TEL)
		{
			foreach ($vCard->TEL as $tel)
			{
				...
				{
					$card->tel[] = $tel['Value'].' ('.implode(', ', $tel['Type']).')';
				}
			}
		}
		...
		if ($vCard->EMAIL)
		{
			foreach ($vCard->EMAIL as $email)
			{
				...
				{
					$card->email[] = $email['Value'].' ('.implode(', ', $email['Type']).')';
				}
			}
		}
		...
		if ($vCard->ADR)
		{
			...
			{
				$addr = new stdClass();
				$addr->address = implode(', ', $address['Type']);
				$addr->street = $address['StreetAddress'];
				$addr->pobox = $address['POBox'];
				$addr->extendedAddress = $address['ExtendedAddress'];
				$addr->locality = $address['Locality'];
				$addr->region = $address['Region'];
				$addr->zip = $address['PostalCode'];
		...
		if ($vCard->AGENT)
		{
			foreach ($vCard->AGENT as $agent)
			{
				...
				{
					$$card->agent[] = carsOutputvCard($agent);
				}
			}
		}
